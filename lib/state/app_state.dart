...
// import '../screens/alarm_screen.dart';   // ✅ [수정] 이 줄이 빠져서 에러가 났습니다. 추가 완료! (Removed duplicate)
...
class AppState extends ChangeNotifier {
  ...
  void toggleMeasurement(BuildContext context) {
    ...
    if (_isMeasuring) {
      ...
      _apneaEvents.clear();
      _sleepStartTime = DateTime.now(); // ✅ 측정 시작 시간 기록 (리포트용)
      ...
      if (_dataCollectionCounter >= 5) {
        ...
        if (_isMeasuring) {
          ...
          if (_bleService != null) {
            ...
            // 만약 0이면 기본값
            if (decibel < 30)
              decibel = 30 + (DateTime.now().millisecond % 10).toDouble();
          }

          _sessionSnoringData
              .add(SnoringDataPoint(DateTime.now(), decibel));
          // print("📝 [DataCollection] 데이터 저장: HR=$_currentHeartRate, dB=$decibel");
        }
      }
      notifyListeners();
    });
  ...
  // ✅ 알람이 이미 표시되었는지 추적하는 변수
  static bool _alarmScreenShown = false;
  static DateTime? _lastAlarmTriggerTime;
  ...
  void _checkAlarmTrigger(BuildContext context) {
    ...
    if (_settingsState!.isExactTimeAlarmOn &&
       ...
       // 같은 분에 여러 번 호출되는 것을 방지
      if (_lastAlarmTriggerTime != null && 
          _lastAlarmTriggerTime!.hour == now.hour &&
          _lastAlarmTriggerTime!.minute == now.minute) {
        return;
      }

      _lastAlarmTriggerTime = now;
      print("⏰ 정각 알람! 강한 진동!");
      
      // ✅ 알람 화면 표시 (이미 표시되지 않았을 때만)
      if (!_alarmScreenShown) {
        _alarmScreenShown = true;
  ...
  // ✅ [새로 추가] 스마트 알람 로직
  void _startSmartAlarmListener(
      BuildContext context, String userId, String sessionId) {
  ...
  void _checkSmartWakeUp(BuildContext context, String currentStage) {
    ...
    if (_settingsState == null || !_settingsState!.isSmartAlarmOn)
      return; // 기능 꺼져있으면 패스
  ...
  void _triggerWakeUpRoutine(BuildContext context) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text("🌅 스마트 알람: 기상 시간입니다! (Light Sleep 감지)"),
  ...
  void _generatePostSleepReport(BuildContext context) {
    ...
    if (settings.isEfficiencyOn) {
      String? efficiencyWarning =
          analyzer.getEfficiencyWarning(finalSleepEfficiency);
    ...
    if (_sleepStartTime != null) {
      ...
      final realMetrics = SleepMetrics(
        ...
        avgSnoringDuration:
            _sessionSnoringData.where((d) => d.decibel > 50).length *
                1.0, // 1분 단위
        avgHrv: 50.0,
        avgHeartRate: _sessionHeartRates.isEmpty
            ? 60.0
            : _sessionHeartRates.reduce((a, b) => a + b) /
                _sessionHeartRates.length,
      ...
      // SleepDataState에 설정
      Provider.of<SleepDataState>(context, listen: false)
          .setTodayMetrics(realMetrics);
      print(
          "✅ [Report] 실제 측정 데이터 리포트 생성 완료 (${_sessionHeartRates.length}분 데이터)");
    }
